        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($image_type, $allowed_types) && $image_size <= $max_file_size) {
                ...
                if (move_uploaded_file($image_tmp, $destination)) {
                    ...
                    if ($stmt->execute()) {
                        // Log action
                        logAction($_SESSION['role'], 'Added Resident named ' . $Res_Lname . ', ' . $Res_Fname . ' ' . $Res_Mname);
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($image_type, $allowed_types) && $image_size <= $max_file_size) {
                ...
                if (move_uploaded_file($image_tmp, $destination)) {
                    ...
                    if ($stmt->execute()) {
                        // Log action
                        logAction($_SESSION['role'], 'Updated Resident named ' . $Res_Lname . ', ' . $Res_Fname . ' ' . $Res_Mname);
